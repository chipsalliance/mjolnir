...
class MjolnirAgent:
    ...
    def get_response_text(self, response) -> str:
        ...
        if prompt_feedback:
            ...
            if block_reason and str(block_reason) not in ["None", "0", "BlockReason.BLOCKED_REASON_UNSPECIFIED"]:
                logger.write(f" [API Warning] Prompt BLOCKED by safety filter. Reason: {block_reason}", stdout=True)
                if hasattr(prompt_feedback, "safety_ratings") and prompt_feedback.safety_ratings:
                    ratings = [f"{r.category}: {r.probability}" for r in prompt_feedback.safety_ratings]
                    logger.write(f" [API Warning] Prompt Safety Ratings: {', '.join(ratings)}", stdout=True)
        ...
        if finish_reason and str(finish_reason) not in ["STOP", "FinishReason.STOP", "1"]:
            logger.write(f" [API Warning] Model execution unfinished. Reason: {finish_reason}", stdout=True)
            if hasattr(candidate, "safety_ratings") and candidate.safety_ratings:
                ratings = [f"{r.category}: {r.probability}" for r in candidate.safety_ratings]
                logger.write(f" [API Warning] Candidate Safety Ratings: {', '.join(ratings)}", stdout=True)
        ...
        if not has_text and has_calls:
            logger.write(" [API Warning] Model session ended with a tool call request but no text parts. The maximum tool execution calls limit (30) was likely reached!", stdout=True)
