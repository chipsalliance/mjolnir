...
let
  mcu-runner = pkgs.writeShellApplication {
    name = "runner";
    ...
    text = ''
      ...
      CODE_DIR="$WORKSPACE_DIR/caliptra-mcu-sw"
      ...
      echo "Using Caliptra MCU source in $CODE_DIR."
      ...
      case "$ACTION" in
        "build")
          echo "Building MCU SW..."
          # shellcheck disable=SC2068
          cargo build "$@"
          ;;
        "test")
          echo "Running MCU SW tests..."
          # shellcheck disable=SC2068
          cargo test "$@"
          ;;
        *)
